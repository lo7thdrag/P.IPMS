...
uses
  ...
  ufrmAlarmSummaryBALL, ufrmAlarmSummaryIPMS, ufrmFireFightingSystem;
...
begin
  ...
  begin
    ...
    begin
      mimic := SysMimics.Mimic.ActiveMimic;
      if Assigned(mimic) and ((mimic.ClassType = TfrmMachinerySpaceVentSystem) or (mimic.ClassType = TfrmFireFightingSystem)) then
      begin
        elemData := SysMimics.Database.GetElement(SysMimics.Mimic.ElementSelected);
        if Assigned(elemData) then
        begin
