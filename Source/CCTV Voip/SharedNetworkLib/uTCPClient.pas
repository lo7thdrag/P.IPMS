...
//==============================================================================
  ...
  public
    ...
    function SendDataF(const aID: word; aBuffer: PAnsiChar): Boolean;
    procedure FlushSendData; override;

  public
    procedure GetPacket();

    function GetSocket: TWSocket;
...
uses
  Windows, Messages;//, uGameData_TTT;
{
...
begin
  ...
  GetMem(FReadBuffer, CSOCK_BUFFER_SIZE);
...
function TTCPClient.SendDataF(const aID: word; aBuffer: PAnsiChar): Boolean;
var pSize: word;
    pid : ^TPacketID;
begin
...
begin
  if WSocket.State = wsConnected then begin
    ...
    FLogStat(DateStr + ': Winsock Send Buffer Size = ' + IntToStr(WSocket.SocketSndBufSize));
    WSocket.SocketSndBufSize:= 1024 * 1024;
    FLogStat(DateStr + ': Winsock Send Buffer Size = ' + IntToStr(WSocket.SocketSndBufSize));

    FLogStat(DateStr + ': Winsock Receive Buffer Size = ' + IntToStr(WSocket.SocketRcvBufSize));
    WSocket.SocketRcvBufSize:= 1024 * 1024;
    FLogStat(DateStr + ': Winsock Receive Buffer Size = ' + IntToStr(WSocket.SocketRcvBufSize));
  end;
end;
...
//  Number of characters in receive buffer but not read yet.
  ...
  ZeroMemory(FReadBuffer, CSOCK_BUFFER_SIZE);
  readByte := TWSocket(Sender).Receive(FReadBuffer, CSOCK_BUFFER_SIZE);
  FLogRecv(TimeStr + ': ReadByte = ' + inttostr(readByte));

  if readByte < 1 then
  begin
    FLogRecv(TimeStr + ': Error Code = ' + inttostr(WSocket_WSAGetLastError));
    Exit;
  end;
  ...
  CopyMemory(PAnsiChar(p), FReadBuffer, readByte);
...
function TTCPClient.GetSocket: TWSocket;
begin
  Result := WSocket;
end;
