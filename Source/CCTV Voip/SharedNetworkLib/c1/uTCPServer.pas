...
//    FLongXAddress: Longword;
  ...
  public
    ...
    function SendDataExceptThisF(const aID: word; aBuffer: PAnsiChar; const ipExcept: string): boolean;
...
function TTCPServer.SendDataExceptThisF(const aID: word; aBuffer: PAnsiChar;
  const ipExcept: string): boolean;
var pSize: word;
    i: integer;
    cCon: TClientConnected;
    strTemp: string;
...
//  FLogRecv(TimeStr + ': ' + inttostr(receivedByte));
  ...
  if readByte < 1 then
  begin
    FLogRecv(TimeStr + ': Error Code = ' + inttostr(TWSocket(Sender).LastError));
    Exit;
  end;
...
function TTCPServer.GetSocket: TWSocket;
begin
  Result := WSocket;
end;

function TTCPServer.getState: TSocketState;
begin
