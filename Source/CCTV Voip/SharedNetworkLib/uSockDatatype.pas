...
//==============================================================================
  ...
  TPacketID = record            //   16 byte header
    recSize     : Word;         //    2 byte size of packet.
    recID       : Word;         //    2 byte record ID.
    ipSender    : LongWord;     //    4 byte original sender,
    ipReceiver  : LongWord;     //    4 byte message destination
    recTime     : LongWord;     //    4 byte time, recording
  end;


const

  CSOCK_BUFFER_SIZE = 2048 * 2048;// 1024 * 1024;
    CSOCK_DEF_PROTO = 'tcp';
