...
uses
  ...
  Dialogs, ExtCtrls, CPort, StdCtrls, uDataType, IniFiles, uListener,
...
type
  ...
  public
    ...
    FRudderIndicator : array of boolean;
    AlarmStop : boolean;
    portAlarmRud : string;
  end;

var
  frmShipAlarm: TfrmShipAlarm;
...
procedure TfrmShipAlarm.SetAllClick(Sender: TObject);
var
  i : Integer;
begin
...
procedure TfrmShipAlarm.CekInput(isVal : Boolean; i : Integer);
var
  j : Integer;
begin
  if isFirst then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            ShipAlarmSystem.sendTelegram(epBoardTelegramKiri, i, True, 2);
            Status := 1;
          end;
        ...
        begin
          ...
          ShipAlarmSystem.sendTelegram(epBoardTelegramKiri, i, True, 1);
          flag := i;
        end;
      end;
    end;
...
procedure TfrmShipAlarm.CheckBox1Click(Sender: TObject);
begin
...
function TfrmShipAlarm.CekBuzzer: boolean;
var
  i : integer;
begin
  if CekAll then
...
procedure TfrmShipAlarm.ComKetikaLoad(Count: Integer);
var
  Str: String;
  S:ShortString;
begin
  ...
  begin
     ...
     {pump ps}
     if s[16]='0' then InputB3.Brush.Color:=cllime else InputB3.Brush.Color:=clGray;
     if s[15]='0' then InputB4.Brush.Color:=cllime else InputB4.Brush.Color:=clGray;
     if s[14]='0' then InputB5.Brush.Color:=cllime else InputB5.Brush.Color:=clGray;
     if s[13]='0' then InputB6.Brush.Color:=cllime else InputB6.Brush.Color:=clGray;
     if s[12]='0' then InputB7.Brush.Color:=cllime else InputB7.Brush.Color:=clGray;
     if s[11]='0' then InputB8.Brush.Color:=cllime else InputB8.Brush.Color:=clGray;
     ...
     if s[19]='0' then InputC8.Brush.Color:=cllime else InputC8.Brush.Color:=clGray;
     ...
     if s[27]='0' then InputD8.Brush.Color:=cllime else InputD8.Brush.Color:=clGray;
...
procedure TfrmShipAlarm.ComKetikaStart(Count: Integer);
var
  Str: String;
  S:ShortString;
begin
  ...
  begin
    ...
    {pump ps : B3-B8}
    if s[16]='0' then begin
      SendPumpStatus(1, 1, 1, True);
      shpInputB3.Brush.Color := clBlue;
    end
    else begin
      FPsPump1[1] := False;
      shpInputB3.Brush.Color := clGray;
    end;

    if s[15]='0' then begin
      SendPumpStatus(1, 1, 2, True);
      shpInputB4.Brush.Color := clBlue;
    end
    else begin
      FPsPump1[2] := False;
      shpInputB4.Brush.Color := clGray;
    end;

    if s[14]='0' then begin
      SendPumpStatus(1, 1, 3, True);
      shpInputB5.Brush.Color := clBlue;
    end
    else begin
      FPsPump1[3] := False;
      shpInputB5.Brush.Color := clGray;
    end;

    if s[13]='0' then begin
      SendPumpStatus(1, 2, 1, True);
      shpInputB6.Brush.Color := clBlue;
    end
    else begin
      FPsPump2[1] := False;
      shpInputB6.Brush.Color := clGray;
    end;

    if s[12]='0' then begin
      SendPumpStatus(1, 2, 2, True);
      shpInputB7.Brush.Color := clBlue;
    end
    else begin
      FPsPump2[2] := False;
      shpInputB7.Brush.Color := clGray;
    end;

    if s[11]='0' then begin
      SendPumpStatus(1, 2, 3, True);
      shpInputB8.Brush.Color := clBlue;
    end
    else begin
      FPsPump2[3] := False;
      shpInputB8.Brush.Color := clGray;
    end;

    {pump ps : C3-C5}
    if s[24]='0' then begin
      SendPumpStatus(1, 3, 1, True);
      shpInputC3.Brush.Color := clBlue;
    end
    else begin
      FPsPump3[1] := False;
      shpInputC3.Brush.Color := clGray;
    end;

    if s[23]='0' then begin
      SendPumpStatus(1, 3, 2, True);
      shpInputC4.Brush.Color := clBlue;
    end
    else begin
      FPsPump3[2] := False;
      shpInputC4.Brush.Color := clGray;
    end;

    if s[22]='0' then begin
      SendPumpStatus(1, 3, 3, True);
      shpInputC5.Brush.Color := clBlue;
    end
    else begin
      FPsPump3[3] := False;
      shpInputC5.Brush.Color := clGray;
    end;

    {pump sb : C6-C8}
    if s[21]='0' then begin
      SendPumpStatus(2, 1, 1, True);
      shpInputC6.Brush.Color := clBlue;
    end
    else begin
      FSbPump1[1] := False;
      shpInputC6.Brush.Color := clGray;
    end;

    if s[20]='0' then begin
      SendPumpStatus(2, 1, 2, True);
      shpInputC7.Brush.Color := clBlue;
    end
    else begin
      FSbPump1[2] := False;
      shpInputC7.Brush.Color := clGray;
    end;

    if s[19]='0' then begin
      SendPumpStatus(2, 1, 3, True);
      shpInputC8.Brush.Color := clBlue;
    end
    else begin
      FSbPump1[3] := False;
      shpInputC8.Brush.Color := clGray;
    end;

    {pump sb : D3-D8}
    if s[32]='0' then begin
      SendPumpStatus(2, 2, 1, True);
      shpInputD3.Brush.Color := clBlue;
    end
    else begin
      FSbPump2[1] := False;
      shpInputD3.Brush.Color := clGray;
    end;

    if s[31]='0' then begin
      SendPumpStatus(2, 2, 2, True);
      shpInputD4.Brush.Color := clBlue;
    end
    else begin
      FSbPump2[2] := False;
      shpInputD4.Brush.Color := clGray;
    end;

    if s[30]='0' then begin
      SendPumpStatus(2, 2, 3, True);
      shpInputD5.Brush.Color := clBlue;
    end
    else begin
      FSbPump2[3] := False;
      shpInputD5.Brush.Color := clGray;
    end;

    if s[29]='0' then begin
      SendPumpStatus(2, 3, 1, True);
      shpInputD6.Brush.Color := clBlue;
    end
    else begin
      FSbPump3[1] := False;
      shpInputD6.Brush.Color := clGray;
    end;

    if s[28]='0' then begin
      SendPumpStatus(2, 3, 2, True);
      shpInputD7.Brush.Color := clBlue;
    end
    else begin
      FSbPump3[2] := False;
      shpInputD7.Brush.Color := clGray;
    end;

    if s[27]='0' then begin
      SendPumpStatus(2, 3, 3, True);
      shpInputD8.Brush.Color := clBlue;
    end
    else begin
      FSbPump3[3] := False;
      shpInputD8.Brush.Color := clGray;
    end;
...
procedure TfrmShipAlarm.ComPortRxChar(Sender: TObject; Count: Integer);
begin
...
procedure TfrmShipAlarm.ConnectClick(Sender: TObject);
begin
...
procedure TfrmShipAlarm.Enabledbtn(val: Boolean);
var
  i: Integer;
begin
  if val then
...
procedure TfrmShipAlarm.EnabledShp(val: Boolean);
var
  i : Integer;
begin
  for i := 0 to ComponentCount - 1 do
...
procedure TfrmShipAlarm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  {Laporan ke instruktur status app}
  ShipAlarmSystem.sendServoLampStatus(C_ORD_LAMP_PS, False);
  Sleep(1000);
end;

procedure TfrmShipAlarm.FormCreate(Sender: TObject);
var
 i, j, k : integer;
begin
  FListener := TListeners.Create;
  with ShipAlarmSystem.Listener.Add('CPP') as TPropertyEventListener do begin
    OnPropertyBoolChange := PCSSystemEvent;
  end;

  SetLength(FLamp, LAMP_COUNT);
  SetLength(FRudderIndicator, 12);
  ...
  begin
    if (i = 2) or (i = 5) or (i = 8) or (i = 11) then
      FRudderIndicator[i] := true
    else
      FRudderIndicator[i] := false;
  end;
...
procedure TfrmShipAlarm.loadSettingAlarm(filepath: string);
  var
    inifile : TIniFile;
...
begin
  ...
  portAlarmRud := inifile.ReadString('Mode', tempstring[2],'Default');
...
procedure TfrmShipAlarm.PanelOutClick(Sender: TObject);
var
 Panel:TPanel;
 newColor:TColor;
begin
...
procedure TfrmShipAlarm.PCSSystemEvent(Sender: TObject; PropsID: E_PropsID;
  Value: Boolean);
begin
  case PropsID of
    epPCSCPPPumpStandby1PS:
      SendPumpStatus(1,1,1,Value);

    epPCSCPPPumpStandby2PS:
      SendPumpStatus(1,2,1,Value);

    epPCSCPPPumpAuto3PS:
      SendPumpStatus(1,3,1,Value);
    ...
    epPCSCPPPumpStandby1SB:
      SendPumpStatus(2,1,1,Value);

    epPCSCPPPumpStandby2SB:
      SendPumpStatus(2,2,1,Value);

    epPCSCPPPumpAuto3SB:
      SendPumpStatus(2,3,1,Value);
...
procedure TfrmShipAlarm.SendPumpStatus(sideId : byte; pumpId, stadeId: Integer; status: Boolean);
begin
  case sideId of
    1 : {port}
    begin
      if pumpId = 1 then
      begin
        if (FPsPump1[stadeId] <> status) then
        begin
          FPsPump1[stadeId] := status;
          ShipAlarmSystem.sendPumpStatus(sideId, pumpId, stadeId, status);
        end;
      end
      else if pumpId = 2 then
      begin
        if FPsPump2[stadeId] <> status then
        begin
          FPsPump2[stadeId] := status;
          ShipAlarmSystem.sendPumpStatus(sideId, pumpId, stadeId, status);
        end;
      end
      else if pumpId = 3 then
      begin
        if FPsPump3[stadeId] <> status then
        begin
          FPsPump3[stadeId] := status;
          ShipAlarmSystem.sendPumpStatus(sideId, pumpId, stadeId, status);
        end;
      end
    end;
    ...
    begin
      if pumpId = 1 then
      begin
        if (FSbPump1[stadeId] <> status) then
        begin
          FSbPump1[stadeId] := status;
          ShipAlarmSystem.sendPumpStatus(sideId, pumpId, stadeId, status);
        end;
      end
      else if pumpId = 2 then
      begin
        if (FSbPump2[stadeId] <> status) then
        begin
          FSbPump2[stadeId] := status;
          ShipAlarmSystem.sendPumpStatus(sideId, pumpId, stadeId, status);
        end;
      end
      else if pumpId = 3 then
      begin
        if (FSbPump3[stadeId] <> status) then
        begin
          FSbPump3[stadeId] := status;
          ShipAlarmSystem.sendPumpStatus(sideId, pumpId, stadeId, status);
        end;
      end
    end;
...
procedure TfrmShipAlarm.setIndikator(i: Integer);
begin
  if i = 1 then
...
procedure TfrmShipAlarm.SettingClick(Sender: TObject);
begin
  ComPort.ShowSetupDialog;
end;

procedure TfrmShipAlarm.tmrBzrTimer(Sender: TObject);
begin
...
var
  MyCounter:Word;
procedure TfrmShipAlarm.tmrComportTimer(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      {Laporan ke instruktur status app}
      ShipAlarmSystem.sendServoLampStatus(C_ORD_LAMP_PS, True);
      counter := 0;
    end;
...
procedure TfrmShipAlarm.TmrKetikaLoad;
var
...
begin
  ...
  {lampu stearing gear}
  LEDH:=0;
  if PanelOutH8.Color=clGray then inc(LEDH,$20);
  if PanelOutH7.Color=clGray then inc(LEDH,$10);
  if PanelOutH6.Color=clGray then inc(LEDH,$08);
  if PanelOutH5.Color=clGray then inc(LEDH,$04);
  if PanelOutH4.Color=clGray then inc(LEDH,$02);
  if PanelOutH3.Color=clGray then inc(LEDH,$01);

  {lampu stearing gear}
  LEDJ:=0;
  if PanelOutJ8.Color=clGray then inc(LEDJ,$20);
  if PanelOutJ7.Color=clGray then inc(LEDJ,$10);
  if PanelOutJ6.Color=clGray then inc(LEDJ,$08);
  if PanelOutJ5.Color=clGray then inc(LEDJ,$04);
  if PanelOutJ4.Color=clGray then inc(LEDJ,$02);
  if PanelOutJ3.Color=clGray then inc(LEDJ,$01);
...
procedure TfrmShipAlarm.TmrKetikaStart;
var
 LEDA,LEDB,LEDC,LEDD:Byte;
 LEDF,LEDH,LEDJ,LEDK:Byte;
 I : Integer;
begin
  ...
  inc(LEDA,$80);
  inc(LEDA,$40);
  {CPP Hydrolic Pump PS}
  {Pump 3}
  if (not FPsPump3[3]) then inc(LEDA,$20);
  if (not FPsPump3[1]) and (not FPsPump3[2]) and (not FPsPump3[3]) then inc(LEDA,$10);
  {Pump 2}
  if (not FPsPump2[3]) then inc(LEDA,$08);
  if (not FPsPump2[1]) and (not FPsPump2[2]) and (not FPsPump2[3]) then inc(LEDA,$04);
  {Pump 1}
  if (not FPsPump1[3]) then inc(LEDA,$02);
  if (not FPsPump1[1]) and (not FPsPump1[2]) and (not FPsPump1[3]) then inc(LEDA,$01);
  {-------}
 ...
 LEDC:=0;
  inc(LEDC,$80);
  inc(LEDC,$40);
  {CPP Hydrolic Pump SB}
  {Pump 3}
  if (not FSbPump3[3]) then inc(LEDC,$20);
  if (not FSbPump3[1]) and (not FSbPump3[2])and (not FSbPump3[3]) then inc(LEDC,$10);
  {Pump 2}
  if (not FSbPump2[3]) then inc(LEDC,$08);
  if (not FSbPump2[1]) and (not FSbPump2[2])and (not FSbPump2[3]) then inc(LEDC,$04);
  {Pump 1}
  if (not FSbPump1[3]) then inc(LEDC,$02);
  if (not FSbPump1[1]) and (not FSbPump1[2])and (not FSbPump1[3]) then inc(LEDC,$01);
  {-------}
 ...
 LEDH:=0;
  inc(LEDH,$80);
  inc(LEDH,$40);
  if not FRudderIndicator[5] then inc(LEDH,$20);
  if not FRudderIndicator[4] then inc(LEDH,$10);
  if not FRudderIndicator[3] then inc(LEDH,$08);
  if not FRudderIndicator[2] then inc(LEDH,$04);
  if not FRudderIndicator[1] then inc(LEDH,$02);
  if not FRudderIndicator[0] then inc(LEDH,$01);

 LEDJ:=0;
  inc(LEDJ,$80);
  inc(LEDJ,$40);
  if not FRudderIndicator[11] then inc(LEDJ,$20);
  if not FRudderIndicator[10] then inc(LEDJ,$10);
  if not FRudderIndicator[9] then inc(LEDJ,$08);
  if not FRudderIndicator[8] then inc(LEDJ,$04);
  if not FRudderIndicator[7] then inc(LEDJ,$02);
  if not FRudderIndicator[6] then inc(LEDJ,$01);
 ...
 LEDK:=0;
  ...
  {inc(LEDK,$01)--> sintak untuk mematikan lampu}
  if (not FTelegrm[1]) or (not FTempTelegrm[1]) then
  begin
   ...
   //yoga kalo  connect baru ngirim data dari software ke hardware
 if ComPort.Connected then
 begin
  ComPort.WriteStr(#$FF+
  Char(LEDA)+Char(LEDB)+Char(LEDC)+Char(LEDD)+
